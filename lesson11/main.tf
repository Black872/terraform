...
resource "aws_autoscaling_group" "webscale" {
  name_prefix         = "WebServer-Highly-Available-ASG-Ver-${aws_launch_template.template.latest_version}"
  min_size            = 2
  max_size            = 2
  min_elb_capacity    = 2
  health_check_type   = "EC2" #Elastic Load Balancer
  vpc_zone_identifier = [aws_default_subnet.default_az1.id, aws_default_subnet.default_az2.id]
  target_group_arns   = [aws_lb_target_group.weblbtargetgp.arn]
  ...
  dynamic "tag" {
    for_each = {
      Name   = "WebServer in ASG-v${aws_launch_template.template.latest_version}"
      TAGKEY = "TAGVALUE"
    }
    content {
      key                 = tag.key
...
# для load balancer нужен target group
resource "aws_lb_target_group" "weblbtargetgp" {
...
resource "aws_lb_listener" "http" {
  ...
  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.weblbtargetgp.arn
  }
}
#-----------------------------------------------------------
